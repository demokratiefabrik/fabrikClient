...
<template>
  ...
  <q-expansion-item
    ...
    :header-class="[
      ...
      'text-h4',
      'q-pa-lg',
      expanded ? 'bg-grey-2' : !peerreviewCompleted ? 'bg-orange' : '',
    ]"
  >
    <template template v-slot:header>
      ...
      <q-item-section>
        ...
        <q-item-label v-if="isInsertProposal">
          Gutachten #{{ firstPeerreview.peerreview.id }}: Vorschlag für eine
          neue Frage
        </q-item-label>
        <q-item-label v-else>
          Gutachten #{{ firstPeerreview.peerreview.id }}:
          {{
        ...
        <q-item-label caption v-if="peerreviewCompleted" class="">
