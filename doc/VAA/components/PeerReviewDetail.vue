...
<template>
  ...
  <q-card flat>
    <span v-if="forceExpanded">
      ...
      <q-toolbar>
        <q-avatar>
          <q-icon size="md" name="mdi-scale-balance"> </q-icon>
          <q-badge
        ...
        <span
          class="finalized_peerreview approved"
          v-if="peerreview.peerreview.approved"
          >Angenommen<br
        /></span>
        <span
          class="finalized_peerreview rejected"
      ...
      <q-card-section> -->
        ...
        <!-- {{peerreviews}} -->
        <span class="text-h4" v-if="isInsertProposal">
          Gutachten #{{ peerreview.peerreview.id }}: Vorschlag für eine neue
          Frage
        </span>
        <span v-else class="text-h4">
          Gutachten #{{ peerreview.peerreview.id }}: Verbesserungsantrag
        </span>
        <div caption v-if="peerreviewCompleted && peerreviewAssigned" class="">
    ...
    <q-tabs
      ...
      narrow-indicator
    >
      <q-tab name="default" icon="mdi-text-search" label="Antrag">
        <q-badge
          color="orange"
      ...
      >
        ...
        >
          <!-- EXPANDABLE-ALL: v-if="isExpanded !== false" -->
          <q-tooltip
            >Die Beiträge in der Diskussion könnten für diese Entscheidung
            relevant sein!</q-tooltip
          >
          !
        </q-badge>
      </q-tab>

      <q-tab name="peerreviews" icon="mdi-scale-balance" label="Verlauf" />
    </q-tabs>
    ...
    <q-tab-panels v-model="tab" animated>
      <q-tab-panel name="default">
        <q-card-section class="text-h4 q-pa-md" v-if="isInsertProposal">
          <div v-if="ownPeerreview">
            Sie haben folgende smartvote-Frage vorgeschlagen:
          </div>
          <div v-if="!ownPeerreview">
            User <i>{{ peerreview.creator.U }}</i> hat folgende smartvote-Frage
            vorgeschlagen:
          </div>
        </q-card-section>
        ...
        >
          <div class="text-h6 text-notification q-ma-md">
            {{ peerreview.content.title }}
          </div>
        </q-card-section>
        <div class="q-pa-md">
          <!-- <div><b>Frage:</b><br>{{peerreview.content.title}}<br><br></div> -->
          <div>
            Die Begründung dazu kann im Register "Diskussion" gelesen und
            kommentiert werden.
          </div>
        </div>

        <q-card-section class="q-mb-md q-ml-md" v-if="!isInsertProposal">
          <!-- <div class="text-h4">Betrifft die smartvote-Frage #{{peerreview.content.id}}</div> -->

          <p v-if="!titleChanges">
            Betrifft die Frage: {{ peerreview.content.title }}
          </p>
          <q-splitter v-model="splitterModel" :horizontal="$q.screen.lt.md">
            <template v-slot:before>
              <div class="q-pa-md">
                ...
                <div class="text-h6 q-mb-md">
                  {{ peerreview.content.title }}<br /><br />
                </div>
            ...
            <template v-slot:after>
              <div class="q-pa-md">
                ...
                <div class="text-h6 q-mb-md">
                  {{ peerreview.peerreview.data_to_apply_on_success.title }}
                  <br /><br />
                </div>
        ...
        <q-card-section v-if="peerreviewFinalized">
          <div class="text-notification">
            Die Entscheidung ist gefallen. Dieser Antrag wurde
            {{ peerreview.peerreview.rejected ? 'abgelehnt' : 'angenommen' }}
          </div>
        ...
        <q-card-section
          class="q-pa-md"
          v-if="
            !peerreviewFinalized && peerreviewAssigned && !peerreviewCompleted
          "
        >
        ...
        >
          ...
          <div>
            ...
            <q-toggle
              ...
              unchecked-icon="mdi-thumb-down"
              :label="
                $t('common_properties.insert.criteria_accept.2', {
            ...
            <SmartvoteCriteria />
            <br />&nbsp;
            <span v-if="!acceptAllAccept"
              ><br />Beantworten Sie die 3 Fragen.</span
            >
          </div>

          <div v-if="acceptAllAccept">
            <span v-if="acceptedNumber < 2" class="text-notification"
              >Sie können diesen Antrag nicht annehmen, weil er mehr als zwei
              Entscheidungskriterien nicht erfüllt.<br
            /></span>
            <span v-else-if="acceptedNumber == 3" class="text-notification"
              >Dieser Antrag ist wohl anzunehmen, denn er scheint alle
              relevanten Entscheidungskriterien zu erfüllen.<br
            /></span>
            <span v-else-if="!accept1" class="text-notification"
              >Sie können diesen Antrag nicht annehmen, da die vorgeschlagene
              smartvote-Frage so oder ähnlich bereits im Fragenkatalog vorhanden
              ist.<br
            /></span>
            <span v-else-if="!accept2" class="text-notification"
              >Sie können diesen Antrag nicht annehmen, da die vorgeschlagene
              smartvote-Frage nicht zum Thema passt.<br
            /></span>
            <span v-else class="text-notification"
              >Sie können diesen Antrag annehmen, auch wenn ein
              Entscheidungskriterien nicht erfüllt ist. Mit
              Verbesserungsanträgen kann dies später noch korrigiert werden.<br
            /></span>
          </div>
        </q-card-section>
        ...
        >
          ...
          <div v-if="acceptAllAccept">
            ...
            <span v-else-if="!accept1" class="text-notification"
              >Sie können diesen Antrag nicht annehmen, da die ursprüngliche
              Idee dieser Frage geändert wird.<br />
            </span>
            <span v-else-if="!accept2" class="text-notification"
              >Sie können diesen Antrag nicht annehmen, da es sich bei diesem
              Vorschlag um keine Verbesserung zu handeln scheint.<br />
            </span>
            <span v-else class="text-notification"
              >Sie können diesen Antrag annehmen, auch wenn ein
              Entscheidungskriterien nicht erfüllt ist. Mit
        ...
        >
          ...
          <p v-if="!peerreviewCompleted">
            Sprechen Sie sich dafür aus, dass dieser Vorschlag angenommen wird?
          </p>
          <p v-if="peerreviewCompleted && !peerreview.progression.response">
            Sie haben sich dafür ausgesprochen, dass dieser Vorschlag abgelehnt
            wird.
          </p>
          <p v-if="peerreviewCompleted && peerreview.progression.response">
            Sie haben sich dafür ausgesprochen, dass dieser Vorschlag angenommen
            wird.
          </p>
        ...
        >
          ...
          <p>
            Wie wichtig ist Ihnen diese Frage für den smartvote-Fragebogen?
            Bitte verschieben Sie den Regler.
          </p>
          ...
          <!-- <p>Wir fragen Sie nun nach Ihrer ganz persönlichen Meinung: </p> -->
          <ContentSalienceSlider :content="content" instruction="" />
        </q-card-section>
      </q-tab-panel>

      <q-tab-panel name="discussion" v-if="tab == 'discussion'">
        <br />
        <div class="text-h6">Diskussion über diesen Antrag</div>
        <p>
          Sie können hier die bisherige Diskussion zum Vorschlag nachverfolgen
          und Ihre Überlegungen mit anderen teilen:
        </p>
        <DefaultDiscussionBlock
          ...
          :discussionBlockLabel="
            isInsertProposal
              ? 'Diskussion zur vorgeschlagenenen Frage'
              : 'Diskussion zum Verbesserungsvorschlag'
          "
          :alwaysExpanded="true"
          :ctx="this"
        />
        <!-- :customAM="AMs.indexTopPeerReview" -->
        <p class="text-notification" v-if="!firstDiscussionContent">
          Das Forum kann aktuell nicht geöffnet werden. Da ist ein Fehler
          aufgetreten.
        </p>
      </q-tab-panel>

      <q-tab-panel name="peerreviews" v-if="tab == 'peerreviews'">
        <PeerReviewHistory
    ...
    <q-card-actions align="right">
      <q-btn icon="mdi-chevron-up" flat right @click="closeExpandedItem"
        >Gutachten schliessen
        <q-tooltip
          >So bekommen Sie wieder den Überblick über die ganze Liste.</q-tooltip
        >
      </q-btn>
    </q-card-actions>
  </q-card>
