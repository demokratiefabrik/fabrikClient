...
<template>
  <q-card flat>
      ...
      <q-card-section avatar>
        ...
        <q-badge
          ...
          >Verbesserung vorgeschlagen<q-tooltip
            >Für diese Frage wurde eine Verbesserung vorgeschlagen, die gerade
            begutachtet wird.
          </q-tooltip>
        </q-badge>
        ...
        <!-- {{peerreviews}} -->
        <span class="text-h4"> {{ node.content.title }} </span>
    ...
    >
      ...
      >
        <q-tooltip v-if="isSalienced(node)"
          >Beschreibung und Bewertung</q-tooltip
        >
        <q-tooltip v-if="!isSalienced(node)"
          >Bitte geben Sie an, wie wichtig Ihnen
          {{
            titlePrefix == 'smartvote-Frage' ? 'die Frage' : 'das Thema'
          }}
          erscheint.</q-tooltip
        >
        <q-badge
      ...
      >
        ...
        <q-badge
          ...
          >{{
            nofDiscussionDescendants ? '' + nofDiscussionDescendants : ''
          }}</q-badge
        >
      </q-tab>
      <q-tab
    ...
    <q-tab-panels v-model="tab" animated>
      ...
      <q-tab-panel name="proposal" v-if="tab == 'proposal'">
        <br />
        <div class="text-h6">Neuer Verbesserungsantrag</div>
        <p>
          Sie möchten eine Verbesserung für die Frage "<span
            class="text-purple"
            >{{ node.content.title }}</span
          >" vorschlagen? Wir sind gespannt!
        </p>

        <div v-if="node.content.pending_peerreview_for_insert">
          Doch leider können Sie für diesen Beitrag noch keinen
          Verbesserungsvorschlag einreichen. Die smartvote-Frage wurde noch
          nicht abschliessend begutachtet. Sie können vorläufig ihr Vorschlag
          als Kommentar im Register "Diskussion" unterbringen. Dann gehen Ihre
          Überlegungen nicht vergessen.
        </div>

        <div v-if="node.content.pending_peerreview_for_update">
          Doch leider wird für diesen Beitrag gerade einen anderen
          Verbesserungsantrag begutachtet. Wir müssen warten bis dieses
          Gutachten abgeschlossen wurde. Möchten Sie Ihre Überlegungen
          stattdessen im Reiter "Diskussion" teilen?
        </div>

        <ProposeVAAQuestionEdit :node="node" v-on:switch-tab="switchTab" />
      </q-tab-panel>
    ...
    <q-card-actions align="right">
      <q-btn
        ...
        <q-tooltip
          >So bekommen Sie wieder einen Überblick über die ganze
          Liste.</q-tooltip
        >
      </q-btn>

      <q-btn flat v-if="asModal" right @click="closeExpandedItem"
        >Schliessen
      </q-btn>
    </q-card-actions>
