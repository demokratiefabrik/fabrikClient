...
<template>
  <div v-if="items">
    <div style="top: 120px; right:0px; max-width:150px; position:fixed; background: rgba(255,255,255,0.3)">
      ...
      <q-list>
        <q-item
          ...
          :class="{'q-pa-none': $q.screen.lt.sm, 'q-ma-none': $q.screen.lt.sm, hidden: (item.visible && !item.visible())}"
        ...
        >
          ...
          <q-item-section
            ...
            :class="{'bg-grey-3': !selectedItemsAnchor.includes(item.anchor) , 'bg-blue-3': selectedItemsAnchor.includes(item.anchor) , 'q-pa-none': true, 'q-ma-none': true}"
          >&nbsp;
            <q-icon
          ...
          <q-item-section v-if="$q.screen.lt.md && $q.screen.gt.xs ">
            <q-icon
              dense
              :class="selectedItemAnchor==item.anchor ? '' : 'bg-white q-pa-xs'"
