...
const getters = {
  ...
  assemblyTuple: (state, getters) => {
    ...
    if (!appComposable.assemblyIdentifier.value) {
      return null
    }

    return (state.assemblydata[appComposable.assemblyIdentifier.value])
  },
  ...
  stageMilestones: (_state, getters) => {
    ...
    const milestones = stateMilestones[appComposable.stageID.value]
    return milestones ? milestones : []
  },

  stageMilestoneLabels: (_state, getters) => {
  ...
  stage: (_state, getters) => {
    const stages = getters.assembly_stages
    return (stages[appComposable.stageID.value])
  },

  assembly_sorted_stages: (_state, getters) => {
...
const actions = {
  ...
  incrementAssemblyActivityCounter({ commit }, { counterName }) {
    ...
    commit('incrementAssemblyActivityCounter', {
      assemblyIdentifier: appComposable.assemblyIdentifier.value,
      counterName
    })
  },
