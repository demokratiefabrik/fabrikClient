<template>
  <span class="float-right q-pa-none q-ma-none">
    ...
    >
      <q-card>
        <q-toolbar>
          <q-toolbar-title>Liste bestehender Fragen zu diesem Thema</q-toolbar-title>
          <q-btn
            flat
            round
            dense
        ...
        <q-card-section>
          <p class="text-caption">Sie finden folgend eine Liste der bereits eingetragenen Fragen zu diesem Thema.</p>
          <ul
            style="max-height: 50vh"
            class="scroll"
          >
            <li
              v-for="node in previousQuestions"
              :key="node.content.id"
            >
              {{node.content.title}}
            </li>
