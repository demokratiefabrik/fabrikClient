<template>
  ...
  >
    ...
    <div v-if="$loaded(topics)">
      ...
      <p>
        Themen, die den Teilnehmenden an der Demokratiefabrik besonders wichtig sind, erhalten mehr Raum im smartvote-Fragenkatalog. Wäre die Könizer-Demokratiefabrik heute schon zu Ende, dann würden die Fragen ungefähr so auf die Themen verteilt:
      ...
      <p v-if="topicHighestSalience && topicHighestSaliencePercent">
        Wenn die Beteiligungswochen bereits beendet wären, dann würde das Thema "{{topicHighestSalience.content.title}}"
    ...
    <div v-if="$loaded(questions)">
      ...
      <p>Im Moment sind {{questions.length}} Fragen im Fragenkatalog der Demokratiefabrik vorhanden. Insgesamt fliessen am Ende etwa 48 Fragen in den finalen smartvote-Fragebogen ein.</p>

      <div v-if="questionsHighestSalience.length > 0">
        <p>
          Hier sind die aktuell 10 am besten bewerteten Fragen in unserem Katalog. Diese haben bisher die grössten Chancen am Ende, so oder in ähnlicher Form, im smartvote-Fragebogen aufgeführt zu werden.</p>
        <br>
        <ul>
          <li
        ...
        <!-- <ContentTreeChart
          chartType="chartBar"
          :hideOrderToggle="true"
          :nodes="questionsHighestSalience"
        /> -->
      </div>
    </div>
    <div v-else>
    ...
    <div v-if="$loaded(peerreviews)">
      ...
      <p>Damit neue Fragen in den Katalog aufgenommen oder bisherige Fragen geändert werden können, müssen die Teilnehmenden Anträge schreiben.</p>
      <br><br>
      <ul>
        <li>Bisher wurden {{peerreviews.length}} Anträge gestellt.</li>
        <li>Davon wurden {{peerreviewsApproved.length}} von anderen Könizerinnen und Könizern angenommen und {{peerreviewsRejected.length}} abgelehnt.</li>
        <li>{{peerreviewsOngoing.length}} werden gegenwärtig begutachtet.</li>
      </ul>
    ...
    <!-- <div>
      ...
      <p>Wir laden insgesamt 9000 Könizerinnen und Könizer in die Demokratiefabrik ein. Wir sind gespannt, wieviele von Ihnen sich für die Demokratiefabrik Interessieren. Aktuell sieht die Beteiligung folgendermassen aus:</p>
      <br><br>
    ...
    <div>
      ...
      <p>Die Demokratiefabrik bleibt noch bis und mit 2. Juli 2021 geöffnet. Wir freuen uns, wenn Sie noch einmal vorbeischauen, um die Arbeiten zum Fragenkatalog erneut zu unterstützen!
        Bei Ihrem nächsten Besuch finden Sie beim Glöckchen oben rechts Informationen dazu, wie andere Teilnehmende auf Ihre Anträge und Kommentare reagiert haben.
