...
<template>
  ...
  >
    ...
    <p
      v-if="assembly"
      v-dompurify-html="assembly.info"
    />
    ...
    <q-card
      :key="item.name"
      v-for="(item, index) in Object.values(menu)"
      flat
      :class="{
        'cursor-pointer': isAccessible(item) || item.expanded(item) || item.expandable, 
        'hovercraft': isAccessible(item) ,
        'bg-blue-1': isFocused(item) && !item.expanded(item)}"
      @click="item.expandable ? (item.manual_expanded = !item.manual_expanded) : clickItem(item)"
    >
      <q-tooltip
        max-width="300px"
      ...
      <q-card-section horizontal>
        ...
        <q-card-section>
          ...
          <div class="text-h5 q-mt-sm q-mb-xs">{{item.toc_label ? item.toc_label : item.label}}</div>
...
export default {
  name: "AssemblyTOC",
  mixins: [StageGroupMixin, AssemblyMixin],
  components: {
    ArtificialModeration,
  },
  data() {
  ...
  methods: {
    ...
    isDisabled(item) {
      return !this.groupsAccessible.includes(item.name);
    },

    isCompleted(item) {
      return !this.groupsScheduled.includes(item.name);
    },

    isScheduledForLater(item) {
      return this.groupsScheduled.includes(item.name) && !this.isFocused(item);
    },
