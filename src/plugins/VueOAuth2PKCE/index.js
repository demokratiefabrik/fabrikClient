...
import { useAuthEmitter } from './oauthEmitter'
...
export default {
  ...
  install(Vue, pkce_config) {
    ...
    Vue.prototype.refresh_token = async function () {
      ...
      this.oauthEmitter.emit('TokenChanges', jwt)
      this.oauthEmitter.emit('AfterTokenChanged', jwt)
    ...
    Vue.prototype.oauth = new Vue({
      ...
      methods: {
        ...
        refresh_token_if_required: async function () {
          ...
          if (await this.expiredJWT()) {
            if (Vue.prototype.getOngoingTokenRefresh()) {
              await Vue.prototype.ensureNoRefreshTokenIsOngoing().catch(_error => { return false })
            } else {
              Vue.prototype.setOngoingTokenRefresh(true)
            }
            if (this.expiredJWT()) {
        ...
        async logout(silent=false) {
          ...
          // console.log(Vue.prototype.pkce.state)
          this.jwt = null
          // this.oauthEmitter.emit('TokenChanges', null)
          // this.oauthEmitter.emit('AfterTokenChanged', null)
          ...
          setTimeout(() => {
            ...
            this.oauthEmitter.emit('AfterTokenChanged', null)
            if (!silent) {
              this.oauthEmitter.emit('AfterLogout')
            }
          }, 10)
        },
        ...
        initialize: async function () {
          ...
          // During Startup
          this.oauthEmitter.on('TokenChanges', jwt => {
            this.jwt = jwt
          ...
          this.oauthEmitter.emit('TokenChanges', token)
          this.oauthEmitter.emit('AfterTokenChanged', token)
          if (hasAuthCode) {
            this.oauthEmitter.emit('AfterLogin')
          }
