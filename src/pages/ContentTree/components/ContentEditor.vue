  ...
  >
    ...
    >
      ...
      <q-btn
        ...
        size="md"
        icon="mdi-close"
        @click.stop="$refs['popup_content_formor'].cancel()"
      />
    ...
    <div
      class="q-pa-md"
      v-if="$loaded(localmodel) && (!CONTENTTREE.limitForAddingCommentsReached ||  isAModification)"
    >
      ...
      <div style="float:right">
        ...
        <q-btn
          ...
          size="md"
          icon="mdi-check"
          @click.stop="$refs['popup_content_formor'].set()"
        />
      </div>

      <div
        class="full-width q-gutter-y-md column"
        style="max-width: 600px"
      >
        <!-- HTML -->
        <div v-if="isAModification">
          <p v-if="reviewNeeded"> Sie können einen Änderungsvorschlag einreichen. Andere zufällig ausgewählte Teilnehmerinnen und Teilnehmer werden danach darüber befinden.</p>
          <p v-if="!reviewNeeded"> Sie können diesen Inhalt direkt bearbeiten oder löschen.</p>
        </div>
        <div v-if="!isAModification">
          <p v-if="reviewNeeded"> Sie können hier einen neuen Beitrag vorschlagen. Andere zufällig ausgewählte Teilnehmerinnen und Teilnehmer werden danach darüber befinden.</p>
          <div v-if="!reviewNeeded">
            <p v-if="parentReference">
              Sie erstellen gerade einen Beitrag zum Thema '{{parentReference}}'. Was möchten Sie mitteilen?
            </p>
            <p v-else>
        ...
        <div v-if="isAModification">
          <b>Beitrag löschen</b>
          Wenn Sie die folgende Checkbox aktivieren, wird dieser Beitrag (und sämtliche Antworten darauf) von der Plattform entfernt.
          <q-toggle
            ...
            color="red"
            :label="localmodel.disabled ? 'Der Beitrag ist im Papierkorb. Möchten Sie ihn wieder anzeigen lassen?' : 'Möchten Sie diesen Beitrag wirklich in den Papierkorb verschieben?'"
          />
        </div>
        ...
        <div v-if="IsManager && isAModification">
          ...
          <q-btn
            ...
            :label="showParentSelectionBool ? `Elternbeiträge aktualisieren` : `Elternbeiträge laden`"
          />
          <!-- && contextNodeTypesOptions.length <= 1 -->
          <q-select
            filled
            ...
            :display-value="`Beitrag: ${currentParent ? `#${currentParent.content.id} ${currentParent.content.title}` : '*Oberste Ebene*'}`"
            :options="parentOptionsFiltered"
            @filter="filterParentsFn"
            :option-value="tuple => tuple ? tuple.content.id : null"
            :option-label="tuple => `${tuple && tuple.content.id==this.localmodel.parent_id ? '*** ' : ''}#${tuple.content.id} ${tuple.content.title}`"
            :option-disable="tuple => tuple ? tuple.content.disabled || tuple.content.deleted : false"
            map-options
            style="width: 250px"
            emit-value
            behavior="menu"
      ...
      <p v-if="!isAModification"> Hinweis: In den Diskussionsforen können Sie pro Tag maximal {{dailyContributionLimits.daylimit}} Beiträge schreiben. <span v-if="dailyContributionLimits.current > 1">Bis jetzt waren es {{dailyContributionLimits.current}}.</span>
      </p>
