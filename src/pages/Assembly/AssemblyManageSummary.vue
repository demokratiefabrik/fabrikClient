<template>
  ...
  >
    ...
    <div v-if="$loaded(questions)">
      ...
      <div v-if="questionsHighestSalience && questionsHighestSalience.length > 0">
        <p>
          Hier sind die aktuell am besten bewerteten Fragen in unserem Katalog. Diese haben bisher die grössten Chancen am Ende, so oder in ähnlicher Form, im smartvote-Fragebogen aufgeführt zu werden.</p>
        <br>
        <ul>
          <li
    ...
    <div v-if="$loaded(peerreviews)">
      ...
      <ul>
        <li>Bisher <span v-if="peerreviews.length!==1">wurden</span><span v-else>wurde</span> {{peerreviews.length}} Anträge gestellt.</li>
        <li v-if="peerreviews.length > 0">Davon <span v-if="peerreviewsApproved.length!==1">wurden</span><span v-else>wurde</span> {{peerreviewsApproved.length}} von anderen Könizerinnen und Könizern angenommen und {{peerreviewsRejected.length}} abgelehnt.</li>
        <li v-if="peerreviews.length > 0">{{peerreviewsOngoing.length}}  <span v-if="peerreviewsApproved.length!==1">werden</span><span v-else>wird</span>  gegenwärtig begutachtet.</li>
      </ul>
